...
class M17AttackVectors:
    ...
    def generate_attack_vectors():
        ...
        # 2. Truncated frames
        # Only 12 bytes
        vectors.append(("truncated_lsf", struct.pack(">H", SYNC_LSF) + b"\x00" * 10))
        # Only 3 bytes
        vectors.append(("truncated_packet", struct.pack(">H", SYNC_PACKET) + b"\x01"))

        # 3. Oversized frames
        vectors.append(
            ("oversized_lsf", M17AttackVectors.create_lsf_frame() + b"\xff" * 100)
        )
        vectors.append(
            ("oversized_packet", M17AttackVectors.create_packet_frame(length=500))
        )
        ...
        vectors.append(
            ("max_size_packet", struct.pack(">H", SYNC_PACKET) + b"\xff" * 328)
        )
        ...
        vectors.append(
            ("sync_bitflip_1", struct.pack(">H", SYNC_LSF ^ 0x0001) + b"\x00" * 46)
        )
        vectors.append(
            ("sync_bitflip_2", struct.pack(">H", SYNC_LSF ^ 0x0100) + b"\x00" * 46)
        )
        vectors.append(
            ("sync_bitflip_3", struct.pack(">H", SYNC_LSF ^ 0x8000) + b"\x00" * 46)
        )
...
def test_m17_deframer_with_vectors():
    ...
    # Test each vector
    results = {"total": len(vectors), "processed": 0, "errors": 0, "warnings": 0}
