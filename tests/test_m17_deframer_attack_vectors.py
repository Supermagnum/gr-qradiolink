...
class M17AttackVectors:
    ...
    def generate_attack_vectors():
        ...
        vectors.append(
            ("truncated_lsf", struct.pack(">H", SYNC_LSF) + b"\x00" * 10, False)
        )
        vectors.append(
            ("truncated_packet", struct.pack(">H", SYNC_PACKET) + b"\x01", False)
        )

        # 3. Oversized frames (deframer extracts first valid frame, which is correct)
        vectors.append(
            ("oversized_lsf", M17AttackVectors.create_lsf_frame() + b"\xff" * 100, True)
        )
        vectors.append(
            ("oversized_packet", M17AttackVectors.create_packet_frame(length=500), True)
        )

        # 4. Invalid sync words (should be rejected)
        vectors.append(
            ("invalid_sync_1", struct.pack(">H", 0x0000) + b"\x00" * 46, False)
        )
        vectors.append(
            ("invalid_sync_2", struct.pack(">H", 0xFFFF) + b"\xff" * 46, False)
        )
        vectors.append(
            ("invalid_sync_3", struct.pack(">H", 0x1234) + b"\xaa" * 46, False)
        )
        ...
        vectors.append(
            (
                "sync_in_payload",
                b"\x00" * 20 + struct.pack(">H", SYNC_LSF) + b"\x00" * 20,
                True,
            )
        )
        ...
        vectors.append(
            (
                "mixed_frames",
                M17AttackVectors.create_lsf_frame()
        ...
        vectors.append(
            ("max_size_packet", struct.pack(">H", SYNC_PACKET) + b"\xff" * 328, True)
        )
        ...
        vectors.append(
            ("null_payload", struct.pack(">H", SYNC_LSF) + b"\x00" * 46, True)
        )
        ...
        vectors.append(
            ("sync_at_end", b"\x00" * 46 + struct.pack(">H", SYNC_LSF), True)
        )
        ...
        vectors.append(
            (
                "special_bytes",
                struct.pack(">H", SYNC_LSF)
                + bytes([0x00, 0xFF, 0x80, 0x7F, 0x01, 0xFE])
                + b"\x00" * 40,
                True,
            )
        )
        ...
        vectors.append(
            ("minimal_packet", struct.pack(">H", SYNC_PACKET) + b"\x01", True)
        )
        ...
        vectors.append(
            (
                "sync_bitflip_1",
                struct.pack(">H", SYNC_LSF ^ 0x0001) + b"\x00" * 46,
                False,
            )
        )
        vectors.append(
            (
                "sync_bitflip_2",
                struct.pack(">H", SYNC_LSF ^ 0x0100) + b"\x00" * 46,
                False,
            )
        )
        vectors.append(
            (
                "sync_bitflip_3",
                struct.pack(">H", SYNC_LSF ^ 0x8000) + b"\x00" * 46,
                False,
            )
        )
        ...
        vectors.append(
            (
                "preamble_frame",
                b"\x00" * 4 + struct.pack(">H", SYNC_LSF) + b"\x00" * 46,
                True,
            )
        )
...
def test_m17_deframer_with_vectors():
    ...
    # Test each vector
    results = {"total": len(vectors), "passed": 0, "failed": 0, "errors": 0}

    for name, vector_data, should_extract in vectors:
        print(f"Testing: {name:30s} ({len(vector_data):4d} bytes)", end=" ... ")

        try:
            passed, message = test_deframer_with_vector(
                name, vector_data, should_extract
            )
            if passed:
                print(f"✓ {message}")
                results["passed"] += 1
