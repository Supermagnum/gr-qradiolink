...
int main(int argc, char** argv)
{
    (void)argc;
    (void)argv;
    std::cout << "Testing mod_bpsk..." << std::endl;
    ...
    auto mod = gr::qradiolink::mod_bpsk::make(125, 250000, 1700, 8000);
    ...
    assert(out.size() == 1000 && "mod_bpsk must produce 1000 complex samples (head limit)");

    std::cout << "mod_bpsk test passed!" << std::endl;
    return 0;
}
