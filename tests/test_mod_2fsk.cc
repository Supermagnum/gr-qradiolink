...
int main(int argc, char** argv)
{
    (void)argc;
    (void)argv;
    std::cout << "Testing mod_2fsk..." << std::endl;
    ...
    auto mod = gr::qradiolink::mod_2fsk::make(125, 250000, 1700, 8000, false);
    ...
    assert(out.size() == 1000 && "mod_2fsk must produce 1000 complex samples (head limit)");

    std::cout << "mod_2fsk test passed!" << std::endl;
    return 0;
}
