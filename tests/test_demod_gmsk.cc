...
BOOST_AUTO_TEST_CASE(test_demod_gmsk_instantiation)
{
    // Use sps=10 which has target_samp_rate=20000, so filter_width <= 10000
    // Using 8000 should be fine for sps=10
    auto demod = demod_gmsk::make(10, 200000, 1700, 8000);
    BOOST_REQUIRE(demod != nullptr);
}

BOOST_AUTO_TEST_CASE(test_demod_gmsk_flowgraph)
{
