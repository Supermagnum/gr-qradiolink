...
#ifndef INCLUDED_QRADIOLINK_DSSS_DESPREDER_CC_H
#define INCLUDED_QRADIOLINK_DSSS_DESPREDER_CC_H
...
/*!
 * \brief DSSS Despreader/Correlator - despreads DSSS signal and recovers symbols
 * \ingroup qradiolink
 *
 * This block accepts spread spectrum signal at chip rate and despreads it
 * to recover the original symbols at symbol rate.
 *
 * Features:
 ...
 * Output: Despread symbols at symbol rate (e.g., 12 ksps)
 *
 * Outputs:
 *  - 0: Despread complex symbols
 *  - 1: Lock status (float, 0.0 = unlocked, 1.0 = locked)
 *  - 2: SNR estimate (float, dB)
 */
class QRADIOLINK_API dsss_despreader_cc : public block
{
public:
    typedef std::shared_ptr<dsss_despreader_cc> sptr;
    ...
    /*!
     * \brief Make a DSSS despreader block
     *
     * \param pn_sequence PN sequence (must match transmitter)
     ...
     */
    static sptr make(const std::vector<int>& pn_sequence,
     ...
     *
     * \param pn_sequence New PN sequence
     */
...
#endif /* INCLUDED_QRADIOLINK_DSSS_DESPREDER_CC_H */
