...
#ifndef GNURADIO4_QRAD_DEMODSOQPSK_HPP
#define GNURADIO4_QRAD_DEMODSOQPSK_HPP
...
GR_REGISTER_BLOCK(gnuradio4::qradiolink::DemodSoqpsk)

struct DemodSoqpsk : gr::Block<DemodSoqpsk> {
    ...
    GR_MAKE_REFLECTABLE(DemodSoqpsk, in, out, samples_per_symbol, frequency_separation_hz);
...
public:
    [[nodiscard]] gr::work::Status processBulk(std::span<const std::complex<float>>& in_span, std::span<std::uint8_t>& out_span) noexcept {
        ...
        const unsigned    bps         = detail::bitsPerSymbolDigital(detail::DigitalKind::SoqpskLike);
        ...
        detail::digitalDemodulateHardBits(detail::DigitalKind::SoqpskLike, in_span, bit_view, sps_local, frequency_separation_hz);
...
#endif // GNURADIO4_QRAD_DEMODSOQPSK_HPP
