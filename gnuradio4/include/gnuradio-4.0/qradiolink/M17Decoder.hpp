...
#ifndef GNURADIO4_QRAD_M17DECODER_HPP
#define GNURADIO4_QRAD_M17DECODER_HPP
...
GR_REGISTER_BLOCK(gnuradio4::qradiolink::M17Decoder)

struct M17Decoder : gr::Block<M17Decoder, gr::NoTagPropagation> {
    using Description = gr::Doc<"PDU shim (deterministic transforms for qa).">;
    gr::MsgPortIn  msg_pdu_in{};
    gr::MsgPortOut msg_pdu_out{};

    GR_MAKE_REFLECTABLE(M17Decoder, msg_pdu_in, msg_pdu_out);
...
public:
    ...
    void processMessages(gr::MsgPortIn& port, std::span<const gr::Message> messages) {
        ...
        for (const auto& msg : messages) {
            ...
            (void)detail::pduApplyM17Decode(pay);
