...
#ifndef GNURADIO4_QRAD_DSTARDECODER_HPP
#define GNURADIO4_QRAD_DSTARDECODER_HPP
...
GR_REGISTER_BLOCK(gnuradio4::qradiolink::DstarDecoder)

struct DstarDecoder : gr::Block<DstarDecoder, gr::NoTagPropagation> {
    using Description = gr::Doc<"PDU shim (deterministic transforms for qa).">;
    gr::MsgPortIn  msg_pdu_in{};
    gr::MsgPortOut msg_pdu_out{};

    GR_MAKE_REFLECTABLE(DstarDecoder, msg_pdu_in, msg_pdu_out);
...
public:
    ...
    void processMessages(gr::MsgPortIn& port, std::span<const gr::Message> messages) {
        ...
        for (const auto& msg : messages) {
            ...
            (void)detail::pduApplyDstDecode(pay);
