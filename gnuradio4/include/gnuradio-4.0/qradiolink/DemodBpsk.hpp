...
#ifndef GNURADIO4_QRAD_DEMODBPSK_HPP
#define GNURADIO4_QRAD_DEMODBPSK_HPP
...
GR_REGISTER_BLOCK(gnuradio4::qradiolink::DemodBpsk)

struct DemodBpsk : gr::Block<DemodBpsk> {
    ...
    GR_MAKE_REFLECTABLE(DemodBpsk, in, out, samples_per_symbol, frequency_separation_hz);
...
public:
    [[nodiscard]] gr::work::Status processBulk(std::span<const std::complex<float>>& in_span, std::span<std::uint8_t>& out_span) noexcept {
        ...
        const unsigned    bps         = detail::bitsPerSymbolDigital(detail::DigitalKind::Bpsk);
        ...
        detail::digitalDemodulateHardBits(detail::DigitalKind::Bpsk, in_span, bit_view, sps_local, frequency_separation_hz);
...
#endif // GNURADIO4_QRAD_DEMODBPSK_HPP
