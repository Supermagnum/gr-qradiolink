...
#ifndef GNURADIO4_QRAD_MODQPSK_HPP
#define GNURADIO4_QRAD_MODQPSK_HPP
...
GR_REGISTER_BLOCK(gnuradio4::qradiolink::ModQpsk)

struct ModQpsk : gr::Block<ModQpsk> {
    ...
    GR_MAKE_REFLECTABLE(ModQpsk, in, out, samples_per_symbol, sample_rate_hz, carrier_offset_hz, frequency_separation_hz);
...
public:
    [[nodiscard]] gr::work::Status processBulk(std::span<const std::uint8_t>& in_span, std::span<std::complex<float>>& out_span) noexcept {
        ...
        detail::digitalModulateWaveform(detail::DigitalKind::Qpsk,
            in_span,
...
#endif // GNURADIO4_QRAD_MODQPSK_HPP
