...
template<typename T>
struct DemodAM : gr::Block<DemodAM<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_am_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<T>                        out{};

    GR_MAKE_REFLECTABLE(DemodAM, in, out);
