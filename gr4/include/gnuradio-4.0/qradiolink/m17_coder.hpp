...
struct M17Coder : gr::Block<M17Coder> {
    using Description = gr::Doc<"GR4 port placeholder — implement from GR3 m17_coder_impl.cc.">;

    gr::PortIn<std::uint8_t>   in{};
    gr::PortOut<std::uint8_t>  out{};

    GR_MAKE_REFLECTABLE(M17Coder, in, out);
