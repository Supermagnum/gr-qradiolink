...
template<typename T>
struct ModCPM4FSK : gr::Block<ModCPM4FSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_cpm_4fsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModCPM4FSK, in, out);
