...
template<typename T>
struct DSSSDespreader : gr::Block<DSSSDespreader<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 dsss__despreader_cc_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DSSSDespreader, in, out);
