...
template<typename T>
struct ModGMSK : gr::Block<ModGMSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_gmsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModGMSK, in, out);
