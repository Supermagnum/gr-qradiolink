...
template<typename T>
struct Demod4FSK : gr::Block<Demod4FSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_4fsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(Demod4FSK, in, out);
