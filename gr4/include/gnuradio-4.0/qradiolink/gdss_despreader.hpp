...
template<typename T>
struct GDSSDespreader : gr::Block<GDSSDespreader<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 gdss__despreader_cc_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(GDSSDespreader, in, out);
