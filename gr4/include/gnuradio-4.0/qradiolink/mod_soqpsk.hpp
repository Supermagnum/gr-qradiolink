...
template<typename T>
struct ModSOQPSK : gr::Block<ModSOQPSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_soqpsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModSOQPSK, in, out);
