...
template<typename T>
struct Mod8FSK : gr::Block<Mod8FSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_8fsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(Mod8FSK, in, out);
