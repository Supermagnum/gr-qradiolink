...
template<typename T>
struct DemodDMR : gr::Block<DemodDMR<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_dmr_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DemodDMR, in, out);
