...
struct M17Decoder : gr::Block<M17Decoder> {
    using Description = gr::Doc<"GR4 port placeholder — implement from GR3 m17_decoder_impl.cc.">;

    gr::PortIn<std::uint8_t>   in{};
    gr::PortOut<std::uint8_t>  out{};

    GR_MAKE_REFLECTABLE(M17Decoder, in, out);
