...
template<typename T>
struct DemodDPMR : gr::Block<DemodDPMR<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_dpmr_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DemodDPMR, in, out);
