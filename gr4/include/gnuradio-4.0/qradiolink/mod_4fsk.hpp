...
template<typename T>
struct Mod4FSK : gr::Block<Mod4FSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_4fsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(Mod4FSK, in, out);
