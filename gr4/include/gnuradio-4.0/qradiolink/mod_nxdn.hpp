...
template<typename T>
struct ModNXDN : gr::Block<ModNXDN<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_nxdn_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModNXDN, in, out);
