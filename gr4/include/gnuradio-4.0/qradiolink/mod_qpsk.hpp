...
template<typename T>
struct ModQPSK : gr::Block<ModQPSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_qpsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModQPSK, in, out);
