...
template<typename T>
struct DemodBPSK : gr::Block<DemodBPSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_bpsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DemodBPSK, in, out);
