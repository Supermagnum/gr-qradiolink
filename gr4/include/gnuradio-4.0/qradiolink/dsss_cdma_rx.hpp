...
template<typename T>
struct DSSSRxCDMA : gr::Block<DSSSRxCDMA<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 dsss__cdma_rx_cc_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DSSSRxCDMA, in, out);
