...
template<typename T>
struct ModBPSK : gr::Block<ModBPSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_bpsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModBPSK, in, out);
