...
template<typename T>
struct DemodQPSK : gr::Block<DemodQPSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_qpsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DemodQPSK, in, out);
