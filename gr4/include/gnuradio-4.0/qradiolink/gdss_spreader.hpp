...
template<typename T>
struct GDSSSpreader : gr::Block<GDSSSpreader<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 gdss__spreader_cc_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(GDSSSpreader, in, out);
