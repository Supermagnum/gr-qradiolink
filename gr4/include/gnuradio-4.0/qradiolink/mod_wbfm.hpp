...
template<typename T>
struct ModWBFM : gr::Block<ModWBFM<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_wbfm_impl.cc.">;

    gr::PortIn<T>                         in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModWBFM, in, out);
