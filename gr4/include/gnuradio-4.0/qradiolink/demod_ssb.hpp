...
template<typename T>
struct DemodSSB : gr::Block<DemodSSB<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_ssb_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<T>                        out{};

    GR_MAKE_REFLECTABLE(DemodSSB, in, out);
