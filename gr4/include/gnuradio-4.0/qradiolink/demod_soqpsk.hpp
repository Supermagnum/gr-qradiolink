...
template<typename T>
struct DemodSOQPSK : gr::Block<DemodSOQPSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_soqpsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DemodSOQPSK, in, out);
