...
template<typename T>
struct DemodNBFM : gr::Block<DemodNBFM<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_nbfm_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<T>                        out{};

    GR_MAKE_REFLECTABLE(DemodNBFM, in, out);
