...
template<typename T>
struct ModNBFM : gr::Block<ModNBFM<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_nbfm_impl.cc.">;

    gr::PortIn<T>                         in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModNBFM, in, out);
