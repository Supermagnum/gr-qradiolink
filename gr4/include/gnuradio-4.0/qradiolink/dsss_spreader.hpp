...
template<typename T>
struct DSSSSpreader : gr::Block<DSSSSpreader<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 dsss__spreader_cc_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DSSSSpreader, in, out);
