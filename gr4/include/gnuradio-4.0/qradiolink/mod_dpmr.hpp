...
template<typename T>
struct ModDPMR : gr::Block<ModDPMR<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 mod_dpmr_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(ModDPMR, in, out);
