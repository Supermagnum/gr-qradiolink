...
template<typename T>
struct Demod8FSK : gr::Block<Demod8FSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_8fsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(Demod8FSK, in, out);
