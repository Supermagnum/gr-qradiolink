...
template<typename T>
struct DemodWBFM : gr::Block<DemodWBFM<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_wbfm_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<T>                        out{};

    GR_MAKE_REFLECTABLE(DemodWBFM, in, out);
