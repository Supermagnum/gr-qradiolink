...
template<typename T>
struct DSSSTxCDMA : gr::Block<DSSSTxCDMA<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 dsss__cdma_tx_cc_impl.cc.">;

    gr::PortIn<std::complex<T>>           in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DSSSTxCDMA, in, out);
