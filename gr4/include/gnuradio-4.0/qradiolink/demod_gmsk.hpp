...
template<typename T>
struct DemodGMSK : gr::Block<DemodGMSK<T>> {
    using Description = gr::Doc<"GR4 port placeholder — implement DSP from GR3 demod_gmsk_impl.cc.">;

    gr::PortIn<std::uint8_t>              in{};
    gr::PortOut<std::complex<T>>          out{};

    GR_MAKE_REFLECTABLE(DemodGMSK, in, out);
