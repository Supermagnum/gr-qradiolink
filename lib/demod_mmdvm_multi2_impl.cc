...
void demod_mmdvm_multi2::set_filter_width(int filter_width)
{
    // Base class implementation - should never be called
    // Actual implementation is in demod_mmdvm_multi2_impl
    (void)filter_width; // Suppress unused parameter warning
}

void demod_mmdvm_multi2::calibrate_rssi(float level)
{
    // Base class implementation - should never be called
...
namespace {
    const std::type_info& g_demod_mmdvm_multi2_typeinfo = typeid(gr::qradiolink::demod_mmdvm_multi2);
    
    struct ForceTypeinfo {
        ForceTypeinfo() {
            const void* ptr = &g_demod_mmdvm_multi2_typeinfo;
            (void)ptr;
        }
    };
    __attribute__((used)) static ForceTypeinfo force_typeinfo;
    
    __attribute__((used)) static const void* force_demod_mmdvm_multi2_typeinfo = &g_demod_mmdvm_multi2_typeinfo;
}
