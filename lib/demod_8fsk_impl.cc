...
demod_8fsk::sptr demod_8fsk::make(int sps, int samp_rate, int carrier_freq, int filter_width, bool fm)
{
    return gnuradio::get_initial_sptr(new demod_8fsk_impl(sps, samp_rate, carrier_freq, filter_width, fm));
}

demod_8fsk_impl::demod_8fsk_impl(int sps, int samp_rate, int carrier_freq, int filter_width, bool fm)
    : demod_8fsk("demod_8fsk",
                 ...
                 gr::io_signature::makev(3, 3, std::vector<int>{sizeof(gr_complex), sizeof(gr_complex), sizeof(char)}))
{
    ...
    // 8FSK constellation: -3.5, -2.5, -1.5, -0.5, 0.5, 1.5, 2.5, 3.5
    std::vector<gr_complex> constellation_points;
    constellation_points.push_back(gr_complex(-3.5, 0));
    constellation_points.push_back(gr_complex(-2.5, 0));
    ...
    gr::digital::constellation_rect::sptr constellation_8fsk = gr::digital::constellation_rect::make(
        constellation_points, pre_diff, 3, 8, 1, 1.0, 1.0);
    ...
    d_symbol_sync = gr::digital::symbol_sync_ff::make(
        ...
        sps_deviation, 1, constellation_8fsk);
...
demod_8fsk_impl::~demod_8fsk_impl() {}
