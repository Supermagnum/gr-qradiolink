...
demod_dpmr::sptr demod_dpmr::make(int sps, int samp_rate)
{
    return gnuradio::get_initial_sptr(new demod_dpmr_impl(sps, samp_rate));
}

demod_dpmr_impl::demod_dpmr_impl(int sps, int samp_rate)
      ...
      d_filter_width(5000.0f)  // 6.25 kHz channel, ~5 kHz filter
{
    ...
    // dPMR 4FSK constellation points
    ...
    // dPMR symbol mapping (reverse of modulator)
    std::vector<int> map;
    ...
    // dPMR descrambler (matches scrambler polynomial)
    // Polynomial: x^15 + x^14 + x^13 + x^11 + 1
    d_descrambler = gr::digital::descrambler_bb::make(0x6801, 0x7FFF, 15);

    // dPMR FEC decoder (convolutional code rate 1/2, constraint length 7)
...
demod_dpmr_impl::~demod_dpmr_impl() {}
