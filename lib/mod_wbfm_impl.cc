...
mod_wbfm::sptr mod_wbfm::make(int sps, int samp_rate, int carrier_freq, int filter_width)
{
    return gnuradio::get_initial_sptr(
        new mod_wbfm_impl(sps, samp_rate, carrier_freq, filter_width));
}

mod_wbfm_impl::mod_wbfm_impl(int sps, int samp_rate, int carrier_freq, int filter_width)
    : mod_wbfm("mod_wbfm",
               ...
               gr::io_signature::make(1, 1, sizeof(gr_complex)))
{
    ...
    d_fm_modulator = gr::analog::frequency_modulator_fc::make(2 * M_PI * d_filter_width / d_if_samp_rate);
    ...
    std::vector<float> if_taps = gr::filter::firdes::low_pass_2(
        25, d_if_samp_rate, d_filter_width, 3500, 60, gr::fft::window::WIN_BLACKMAN_HARRIS);
    d_audio_resampler = gr::filter::rational_resampler_fff::make(25, 1, if_taps);

    std::vector<float> interp_taps = gr::filter::firdes::low_pass_2(
        d_sps, d_samp_rate, d_filter_width, d_filter_width / 2, 60, gr::fft::window::WIN_BLACKMAN_HARRIS);
    ...
    d_filter = gr::filter::fft_filter_ccf::make(
        1,
        gr::filter::firdes::low_pass_2(
            1, d_if_samp_rate, d_filter_width, d_filter_width / 2, 60, gr::fft::window::WIN_BLACKMAN_HARRIS));

    connect(self(), 0, d_audio_filter, 0);
    connect(d_audio_filter, 0, d_audio_amplify, 0);
    connect(d_audio_amplify, 0, d_pre_emph_filter, 0);
    connect(d_pre_emph_filter, 0, d_audio_resampler, 0);
    connect(d_audio_resampler, 0, d_fm_modulator, 0);
...
mod_wbfm_impl::~mod_wbfm_impl() {}

void mod_wbfm_impl::set_filter_width(int filter_width)
{
    d_filter_width = filter_width;
    std::vector<float> if_taps = gr::filter::firdes::low_pass_2(
        25, d_if_samp_rate, d_filter_width, 3500, 60, gr::fft::window::WIN_BLACKMAN_HARRIS);
    std::vector<float> filter_taps = gr::filter::firdes::low_pass_2(
        1, d_if_samp_rate, d_filter_width, 1200, 60, gr::fft::window::WIN_BLACKMAN_HARRIS);
    std::vector<float> interp_taps = gr::filter::firdes::low_pass_2(
        d_sps, d_samp_rate, d_filter_width, d_filter_width, 60, gr::fft::window::WIN_BLACKMAN_HARRIS);

    d_audio_resampler->set_taps(if_taps);
    d_filter->set_taps(filter_taps);
    d_resampler->set_taps(interp_taps);
    d_fm_modulator->set_sensitivity(2 * M_PI * d_filter_width / d_if_samp_rate);
}

void mod_wbfm_impl::set_bb_gain(float value) { d_bb_gain->set_k(value); }
