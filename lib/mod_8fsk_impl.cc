...
mod_8fsk::sptr mod_8fsk::make(int sps,
                               int samp_rate,
                               int carrier_freq,
                               int filter_width,
                               bool fm)
{
    return gnuradio::get_initial_sptr(
        new mod_8fsk_impl(sps, samp_rate, carrier_freq, filter_width, fm));
}

mod_8fsk_impl::mod_8fsk_impl(int sps,
                               int samp_rate,
                               int carrier_freq,
                               int filter_width,
                               bool fm)
               ...
               gr::io_signature::make(1, 1, sizeof(gr_complex)))
{
    ...
    constellation.push_back(-3.5);
    constellation.push_back(-2.5);
    ...
    d_packer = gr::blocks::pack_k_bits_bb::make(3);  // Pack 3 bits for 8FSK
...
mod_8fsk_impl::~mod_8fsk_impl() {}

void mod_8fsk_impl::set_bb_gain(float value) { d_bb_gain->set_k(value); }

void mod_8fsk::set_bb_gain(float value)
{
    // This should never be called, as mod_8fsk is only an interface
    // The actual implementation is in mod_8fsk_impl
}
