...
demod_gmsk::sptr demod_gmsk::make(int sps, int samp_rate, int carrier_freq, int filter_width)
{
    return gnuradio::get_initial_sptr(new demod_gmsk_impl(sps, samp_rate, carrier_freq, filter_width));
}

demod_gmsk_impl::demod_gmsk_impl(int sps, int samp_rate, int carrier_freq, int filter_width)
    : demod_gmsk("demod_gmsk",
                 ...
                 gr::io_signature::makev(4, 4, std::vector<int>{sizeof(gr_complex), sizeof(gr_complex), sizeof(char), sizeof(char)}))
{
    ...
    d_symbol_sync = gr::digital::symbol_sync_ff::make(
        ...
        sps_deviation, 1, gr::digital::constellation_bpsk::make());

    d_freq_demod = gr::analog::quadrature_demod_cf::make(d_samples_per_symbol / (M_PI / 2));
    d_symbol_filter = gr::filter::fft_filter_fff::make(
        1,
        gr::filter::firdes::low_pass(
            1, d_target_samp_rate, d_target_samp_rate / d_samples_per_symbol,
            d_target_samp_rate / d_samples_per_symbol));
    ...
    connect(d_freq_demod, 0, d_symbol_filter, 0);
...
demod_gmsk_impl::~demod_gmsk_impl() {}
